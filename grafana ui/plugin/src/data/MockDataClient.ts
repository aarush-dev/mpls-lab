...
import { synthSeries, tunnelStressAt } from './telemetrySynth';
...
// Connection count per node (physical + tunnel links), for degree-scaled stress thresholds.
const degreeOf = new Map<string, number>();
for (const l of topology.links) {
  degreeOf.set(l.source, (degreeOf.get(l.source) ?? 0) + 1);
  degreeOf.set(l.target, (degreeOf.get(l.target) ?? 0) + 1);
}
...
export class MockDataClient implements DataClient {
  ...
  private nodeStateAt(deviceId: string): 'red' | 'amber' | 'yellow' | 'green' {
    ...
    const degree = degreeOf.get(deviceId) ?? 1;
    if (tunnelStressAt(deviceId, role, this.cursor, degree)) {
      return 'yellow';
    }
    return 'green';
  }
