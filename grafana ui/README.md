...
- **Copilot** (`CopilotPage.tsx`, `/copilot`) — chat-style assistant over incidents/telemetry. Being wired to the real streaming copilot `/chat` (#66); the data-layer seam (`DataClient.chat`, T1/#67) is done — the page itself is a placeholder until T2/#68 rebuilds it on that seam. No mock path exists.
