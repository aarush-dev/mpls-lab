...
**t_impact**  
The timestamp when the fault became observable in telemetry, and one of four `impact_method` values in the label row (`faults/orchestrator.py:585-603`):
...
**lead_time_s**  
The time delta `t_impact - t_start`, in seconds, **floored at `4 * step`** (120 s at the default 30 s step) regardless of the calibrated value (`synthetic/generate.py:13`). This is the "warning window" for the ML model: "we injected a fault at time T; the user felt impact at T + lead_time. Can your model predict the impact by time T + lead_time?" A large lead time means the model has a long precursor window (e.g., latency creep, jitter increase) to learn from. The floor exists because a lead time shorter than a few buckets gives the model no usable precursor window at all — the ML team is looking for **maximum lead time**, i.e. can you predict as early as possible.
