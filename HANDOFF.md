...
- **`frontend/`** — Grafana **App Plugin** (`mplslab-noccopilot-app`, React+TS) over the committed sample data. **Scope now: frontend + mock data only** — no live API/backend. Self-contained shareable folder: plugin source, folder-local Docker Compose (`grafana/grafana:11.1.0`, unsigned-allow, air-gap flags), provisioning, fixtures, docs. **Mock mode** (bundled fixtures, no lab) is the only built mode; live **api mode** (maps the 6 real `dataapi` endpoints) is future work kept cheap by the DataClient seam. 7 sections (Overview, data-driven expandable Topology, in-Grafana Node Details, Telemetry, Incidents/Predictions, mock Copilot, Data/Integration Status). All fabricated predictions/chat are `source:'mock'` + Demo marker. Plan: `frontend/IMPLEMENTATION_PLAN.md`. **Status: M1–M3 DONE, M4 (copilot UI) next.** M2 delivered `frontend/scripts/generate_fixtures.py` (deterministic, verified byte-identical rerun) and `frontend/plugin/src/fixtures/*.json` (bucketCount=152, deviceIds=70, incidents=28, predictions=69, faultTypes=21, topologyNodes=148, topologyLinks=361), `MockDataClient`, global demo clock, `PlaybackControls`/`AppShell`/`MetricCard`, live OverviewPage. M3 delivered the remaining 5 operator pages: Topology (live cytoscape map, POP clustering, role→style registry with default fallback, click node → Node Detail), Node Detail (`/node/:id`, preserves demo clock), Telemetry Explorer (fault-overlay bands), Incidents & Predictions (risk-ordered, error-not-empty), Data & Integration Status. Global FilterBar (POP+device) wired into the reducer. Gate met: click node → Node Detail w/ preserved clock; new fixture node renders with no code change. M4 delivered the Copilot page (`CopilotPage.tsx`+`CopilotChat.tsx`, was a stub): message state machine draft→sending→complete/error→retry, client-generated message ids (counter ref, deterministic), send disabled while in-flight, context = top live incident at the current cursor kept in sync with the demo clock, all 21 fault types have a seeded reply with citations/evidence/root-cause/actions as a structured card, citations restricted to the 4 existing `ragcorpus/*.md` docs. Not added: 3 optional new ragcorpus runbook stubs (unreferenced by fixtures, YAGNI). All 7 pages now implemented. Build green, `tsc --noEmit` clean, plugin.json 1.0.5, serves in Grafana 11.1.0 at `/a/mplslab-noccopilot-app`. **Status: M1–M4 DONE, M5 (QA/handoff) next.**
...
13. **The real capture's labels were re-joined, not re-measured** (`dataapi/reschema.py`), which is why its fault rows went 327 -> 391. Its `impact_methods` contain no `ramp_derived` because the capture predates it.
