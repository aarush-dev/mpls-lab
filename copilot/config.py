...
Field provenance (see docs/adr/):
  llm_profile / embed_profile   ADR-0004 / ADR-0006  (llm: nim|gemma|unsloth-local; embed: nim|unsloth-local)
...
_LLM_PROFILES = _EMBED_PROFILES | {"gemma"}  # gemma is LLM-only -- no embedding endpoint
...
class Config:
    ...
    llm_profile: str = "nim"          # nim (interim) | gemma (on-prem) | unsloth-local (air-gapped)
    ...
    empty_answer_max_retries: int = 2  # #126: separate budget -- an empty terminal turn is a
                                      # model glitch, not a gate-fail evidence deficiency
