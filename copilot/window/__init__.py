"""copilot.window -- Lane-Runtime (R3). WindowContext {start,end,frozen}: live/query/forensic (ADR-0002).

The investigation window the loop threads into every tool call (the model never sets it,
ADR-0002/0015). One struct, three resolutions:
...
X = cfg.window_x_min (default 10, ADR-0002). start<end is enforced downstream by
