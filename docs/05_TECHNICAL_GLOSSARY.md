...
**Promtail**  
The agent that ships logs to Loki. In this lab, Promtail listens on syslog port 1514 (both TCP and UDP), parses FRR messages, attaches labels such as `device=ce_branch1`, and sends them to Loki.

**Grafana**  
A visualization and dashboarding tool. Grafana can query Prometheus/VictoriaMetrics for metrics and Loki for logs. Active plugin Grafana provisions **NOC Overview** with 11 metric panels and **Router Logs** with one Loki Logs panel, a device dropdown, and 5-second near-live polling (`grafana ui/grafana/dashboards/`). For a true live tail, use Grafana Explore with Loki's **Live** mode.
...
- `modelled` — a fixed delay after `t_start`, used where no clean impact metric exists (BGP flap, node kill, `bgp_cascade`, and `brownout` — no probe, since a rate cap has no direct telemetry-path observable; the small delay/loss it now installs is a real impairment the calibrated overlay ramps).
